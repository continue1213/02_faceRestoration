...
class ZSSR_lightning(pl.LightningModule):
    ...
    def calculate_score(self, x, y):
        clipiqa = self.clipiqa(x, y) if self.clipiqa is not None else 0
        maniqa = self.maniqa(x, y) if self.maniqa is not None else 0
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if score > self.max_score:
            ...
            print("max_score has changed into: ", self.max_score)
